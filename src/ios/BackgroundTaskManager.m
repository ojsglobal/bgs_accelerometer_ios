...
-(UIBackgroundTaskIdentifier)beginNewBackgroundTask
{
    ...
    if([application respondsToSelector:@selector(beginBackgroundTaskWithExpirationHandler:)]){
        bgTaskId = [application beginBackgroundTaskWithExpirationHandler:^{
           // DLog(@"background task %lu expired", (unsigned long)bgTaskId);
        }];
        if ( self.masterTaskId == UIBackgroundTaskInvalid )
        {
            self.masterTaskId = bgTaskId;
            //DLog(@"started master task %lu", (unsigned long)self.masterTaskId);
        }
        else
        {
...
-(void)drainBGTaskList:(BOOL)all
{
    ...
    if([application respondsToSelector:@selector(endBackgroundTask:)]){
        ...
        for ( NSUInteger i=(all?0:1); i<count; i++ )
        {
           ...
           // DLog(@"ending background task with id -%lu", (unsigned long)bgTaskId);
            [application endBackgroundTask:bgTaskId];
            [self.bgTaskIdList removeObjectAtIndex:0];
        }
        if ( self.bgTaskIdList.count > 0 )
        {
           // DLog(@"kept background task id %@", [self.bgTaskIdList objectAtIndex:0]);
        }
        if ( all )
        {
           // DLog(@"no more background tasks running");
            [application endBackgroundTask:self.masterTaskId];
            self.masterTaskId = UIBackgroundTaskInvalid;
        }
        else
        {
           // DLog(@"kept master background task id %lu", (unsigned long)self.masterTaskId);
        }
    }
}
